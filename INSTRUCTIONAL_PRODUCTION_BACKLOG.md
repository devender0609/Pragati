# Instructional production backlog
...
- `pragati_iu_g01_ch01_u1` — Where is it? Position words (NEEDS_HUMAN_CHECK, flagged for human check). 
- `pragati_iu_g01_ch01_u2` — Before, after and between in a line (NEEDS_HUMAN_CHECK, flagged for human check). 
- `pragati_iu_g01_ch01_u3` — Sorting things into groups (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g01_ch02_u1` — Long things and round things (NEEDS_HUMAN_CHECK, flagged for human check). 
...
- `pragati_iu_g02_ch01_u4` — Trays, groups and leftovers (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch02_u1` — Solid shapes around us (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch02_u2` — Faces, edges and corners (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch03_u1` — Numbers on the bead string and number strip (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch03_u2` — Skip counting and jumps (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch03_u3` — Patterns in the number chart (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch04_u1` — Shadows and flat shapes (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch04_u2` — Telling shapes apart by their properties (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch04_u3` — Making patterns with shapes (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch05_u1` — Kinds of lines (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch05_u2` — Drawing with lines (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch06_u1` — Adding tens and ones without regrouping (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch06_u2` — Adding on a bead string and a number line (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch06_u3` — Adding with regrouping (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch06_u4` — Taking away two-digit numbers (DRAFT_DECOMPOSITION, flagged for human check). Pages beyond 59 of this chapter were not read in this pass; further subtraction units may be needed.
- `pragati_iu_g02_ch07_u1` — Why we need the same unit (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch07_u2` — Measuring with a chosen unit (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch07_u3` — Heavier, lighter and a simple balance (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch08_u1` — Equal groups and "times" (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch08_u2` — The multiplication sign (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch08_u3` — Tables of 2, 3, 5 and 10 (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch08_u4` — Using multiplication in pictures and stories (DRAFT_DECOMPOSITION, flagged for human check). Pages beyond 90 were not read in this pass.
- `pragati_iu_g02_ch09_u1` — Seasons and the year (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch09_u2` — Reading a calendar (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch09_u3` — How long does it take? (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch09_u4` — Telling the time of day (DRAFT_DECOMPOSITION, flagged for human check). Pages beyond 105 were not read in this pass; the clock work may extend further.
- `pragati_iu_g02_ch10_u1` — Money: notes, coins and totals (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch10_u2` — Paise and making a rupee (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch10_u3` — Spending, change and different ways to pay (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch11_u1` — Collecting data into a table (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch11_u2` — Showing data in a picture chart (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g02_ch11_u3` — Asking your own question and collecting answers (DRAFT_DECOMPOSITION, flagged for human check). 
- `pragati_iu_g01_ch03_u4` — Counting in a busy picture, and two groups that make a number (NEEDS_HUMAN_CHECK, flagged for human check). 
