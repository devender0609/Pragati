...
import { TeacherPanel, AvailabilityRow } from './design/TeacherKit';
          ...
          (() => {
            ...
            return (
              <StudentRouteOutlet
                ...
                // v0.78 §5 — END-TO-END ASSIGNMENT, THE DEFECT.
                //
                // `AssessmentAssignment` has said since v0.11 that the
                // title is "surfaced on the student's assignment card"
                // and that setting `active: false` means "the student
                // home no longer surfaces it". No such card existed
                // anywhere in the student product. A teacher could
                // create an assignment, see it listed under Assign, and
                // no student would ever be shown it.
                //
                // Nobody caught it because every test exercised the
                // teacher form and the storage layer, and both worked.
                // The missing piece was the last hop, which is exactly
                // what §5 asks to verify rather than assume.
                //
                // This qualifies under the Student lock as a functional
                // defect: the fix restores a capability the product
                // claims, and changes no locked composition beyond
                // adding the band the claim requires.
                assignment={(() => {
                  const a = loadAssignments().find((x) => x.active);
                  return a
                    ? { id: a.id, title: a.title, note: a.teacherNote }
                    : null;
                })()}
                onStartAssignment={(id) => {
                  const a = loadAssignments().find((x) => x.id === id);
                  if (!a || !activeStudent) return;
                  startAssessmentFor(
                    activeStudent,
                    'baseline',
                    a.skillMode,
        ...
        {view === 'teacherLanding' && (
          <TeacherShell
            ...
            onSwitchTab={(t) => {
              ...
              // v0.79 §11 — EVERY DESTINATION STAYS IN THE WORKSPACE.
              //
              // Classes and Assign used to leave the shell entirely:
              // selecting them set a top-level view, so the teacher lost
              // the rail, the class scope and the navigation, and landed
              // on a page whose only way back was a "← Back to teacher
              // home" link. Two of six destinations were a different
              // product.
              //
              // They are now tab bodies like the other four. The old
              // top-level routes still exist for deep links and for the
              // student-detail flow, but the normal path never uses them.
              setTeacherTab(t);
          ...
          >
            {teacherTab === 'overview' && (
              <TeacherOverviewBody
                ...
                // v0.78 §11 — these two still jumped to the standalone
                // top-level routes, so Overview's own buttons left the
                // workspace even after the tab switcher was fixed. A
                // teacher pressing "Open Classes" lost the rail and the
                // class scope. Both now switch tabs.
                onOpenAssign={() => setTeacherTab('assign')}
                onOpenClasses={() => setTeacherTab('classes')}
            ...
            {teacherTab === 'assess' && (
              ...
              {/* v0.78 §15 — TWO TRACKS, NAMED BEFORE EITHER IS OFFERED.
                  A teacher arriving here needs to know which of these
                  they are looking at before they read a control. An
                  instructional check is classroom evidence a teacher
                  builds and interprets themselves; Pragati Growth is a
                  governed instrument with calibration and norming
                  claims attached. Letting the first inherit the second's
                  vocabulary is how unsupported measurement claims get
                  made by accident, so the distinction is stated on the
            ...
            {teacherTab === 'resources' &&
              (teacherChapterId ? (
                ...
                <div className="space-y-8">
                  {/*
                    v0.78 §13 — WHOSE SCREEN IS THIS?

                    v0.62 moved the coverage matrix out of Admin because
                    it was trapped there, and v0.68 added the Classes
                    1-12 completeness table so a teacher could see the
                    whole official structure. Both were right about the
                    problem and wrong about the destination: the result
                    was that a teacher opening Resources met a twelve-row
                    matrix of UNKNOWN cells, "Official structure not yet
                    verified", and columns headed "In Pragati",
                    "Reviewed" and "Complete drafts" before reaching a
                    single thing they could teach.

                    Those are governance questions — is our evidence and
                    review state valid — and the person who asks them is
                    an administrator. They keep their full detail under
                    Admin & Research. What a teacher needs from the same
                    facts is which curriculum is verified and what
                  ...
                  */}
                  ...
                  >
                    <button
                      type="button"
                      onClick={() => setView('curriculumCoverage')}
                      className="tap rounded-full bg-white px-5 py-2.5 text-sm font-bold text-ink-800 ring-1 ring-ink-200 transition hover:bg-paper-100"
                    >
