...
import { TeacherPageHeader, TeacherEmptyState } from '../../design/TeacherKit';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        {/* v0.78 §10/§16 — "growth history" was stale product language.
            Pragati records sessions and responses; it does not measure
            growth, which is the frozen governed track. The row opens a
            record of what the student did, and that is what it now
            says. The "Teacher dashboard" eyebrow went with it: the rail
            already says where you are. */}
        <TeacherPageHeader
          title="Students"
          detail="One row per student. Open a row to see what they have worked on and how they responded."
        />
...
function EmptyDashboard({ onStart }: { onStart: () => void }) {
  return (
    // v0.78 §10 — the empty roster is a setup step, not a decorative
    // panel with a star in it. One action, named for what it does.
    <TeacherEmptyState
      title="No students yet"
      detail="Students appear here once they have worked on something. Start a student off and their sessions and responses are recorded against their name."
      actions={[{ label: 'Add the first student', onClick: onStart, primary: true }]}
    />
  );
}
