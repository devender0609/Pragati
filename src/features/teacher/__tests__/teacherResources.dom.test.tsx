...
describe('§9 the teacher never leaves TeacherShell', () => {
  it('the resource list renders inside the shell chrome', () => {
    ...
    expect(screen.getAllByRole('tab', { name: /insights/i }).length)
      .toBeGreaterThan(0);
    // v0.78 — the heading is now "Curriculum" and the word also appears
    // in the depth panel beneath it, so match the heading specifically.
    expect(
      screen.getAllByText(/Curriculum/i).length
    ).toBeGreaterThan(0);
  });
...
describe('§9 the resource page exposes what actually exists', () => {
  ...
  it('does not claim curriculum verification that has not happened', () => {
    ...
    // v0.61 §9 — the Fractions record is now primary_source_verified:
    // the chapter number and title were read from the NCERT textbook
    // itself. The page must say exactly THAT, and must still stop short
    // of claiming teacher verification, which has not happened.
    expect(screen.getByText(/Checked against the official textbook/i)).toBeTruthy();
    expect(screen.queryByText(/^Verified by a subject teacher$/i)).toBeNull();
    // And it must still disclose what was not checked.
    expect(screen.getByText(/What was and was not checked/i)).toBeTruthy();
    expect(
      screen.getByText(/subject teacher has not yet confirmed/i)
    ).toBeTruthy();
  });
