...
export type StudentHomeViewProps = {
  ...
  /**
   * v0.78 §5 — what the teacher assigned, when something is live.
   *
   * Restored capability, not new design: the assignment type has always
   * said its title is "surfaced on the student's assignment card", and
   * no card existed. This is that card, in the studio register.
   */
  assignment: { title: string; note: string; onStart: () => void } | null;
      ...
      <StudioField accent={accent} className="-mx-3 pb-40 sm:-mx-4 sm:rounded-[2rem] lg:pb-48">
        <div className="grid items-center gap-8 px-6 pt-8 lg:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)] lg:gap-14 lg:px-10 lg:pt-10">
          <div>
            ...
            {assignment ? (
              // Named as what it is: a thing the teacher set. It sits
              // above the chapter action because a student who has been
              // given something should be told so before being offered
              // something else.
              <div className="mt-6 rounded-2xl bg-white/[0.07] p-5">
                <p className="text-sm font-semibold text-saffron-300">
                  Your teacher set this
                </p>
                ...
                <button
                  type="button"
                  onClick={assignment.onStart}
                  className="tap mt-4 rounded-full bg-white px-6 py-2.5 font-display text-sm font-bold text-ink-950"
                >
                  Start it
                </button>
              </div>
