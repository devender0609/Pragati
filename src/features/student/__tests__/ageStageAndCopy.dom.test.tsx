...
describe('§5 student screens carry no development jargon', () => {
  ...
  it('the teacher footer still carries the ethical limitation', () => {
    // v0.71 §16 — the WORDING changed, the CLAIM did not. The footer used
    // to read "Pragati prototype · Pre-pilot content · Not a calibrated
    // assessment...". The maturity commentary left a teacher's daily
    // chrome; the substantive limitation — no calibrated score, teacher
    // judgement required, calibration study before operational use — is
    // still stated in full, and this test guards that, not the phrasing.
    render(<Footer appMode="teacher" />);
    expect(screen.getByText(/does not produce a calibrated score/i)).toBeTruthy();
    expect(screen.getByText(/calibration study/i)).toBeTruthy();
  });
