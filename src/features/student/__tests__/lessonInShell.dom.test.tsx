...
describe('§1 the lesson renders inside the shell', () => {
  ...
  it('shows the lesson content itself, not just the chrome', () => {
    renderOutlet({ location: LESSON });
    // v0.77 §11 — this asserted the skill code "FR.02" was on screen,
    // as a cheap proof that the lesson body had rendered. The code is
    // now deliberately absent from the STUDENT lesson: it is our filing
    // system, not a thing a Class 6 student should read. The assertion
    // moves to the lesson's actual content, which is what it was really
    // trying to check.
    expect(screen.getAllByText(/Learn the idea/i).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/Worked examples/i).length).toBeGreaterThan(0);
  });
