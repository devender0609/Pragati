...
export type StudentRouteOutletProps = {
  /** v0.62 §3 — replaces the Learn tab body for grades whose official
   *  curriculum structure is primary-verified. Only Class 6 supplies
   *  it today; other grades keep the legacy module list rather than
   *  have an official structure invented for them. */
  learnOverride?: React.ReactNode;
  /** v0.64 §5 — official section navigation. */
   ...
   *
   * Ordinary assignments had no route to the student at all. The
   * teacher form worked, storage worked, and the last hop did not
   * exist.
   */
  assignment?: { id: string; title: string; note: string } | null;
  onStartAssignment?: (assignmentId: string) => void;
  /** §9 — live formal assignment, if any. */
...
export function StudentRouteOutlet(props: StudentRouteOutletProps) {
  const {
    ...
    growthAssignment, growthInProgress, onStartGrowthCheck,
    learnOverride,
    onOpenSection,
  } = props;
  ...
  return (
    <StudentShell
      ...
      growthAssignment={growthAssignment}
      growthInProgress={growthInProgress}
