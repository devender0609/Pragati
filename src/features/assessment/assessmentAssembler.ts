...
export type PilotAdministrationSpecification = {
  ...
  // v0.58 §3 — lifecycle. Only 'approved_for_field_test' may create a
  // student assignment; see mayAdministerSpecification().
  status:
    | 'draft'
    | 'framework_review'
    | 'expert_review'
...
// ---------------------------------------------------------------------------
// v0.58 §15 — Which blueprint fields are actually enforced?
//
// A specification field that looks like a constraint but is never
// checked is worse than an absent one: it implies a guarantee the
// assembler does not provide. This table is the honest answer, and a
// test asserts it matches behaviour.
// ---------------------------------------------------------------------------

export type ConstraintEnforcement = 'hard' | 'advisory' | 'documentation_only';

export const BLUEPRINT_CONSTRAINT_ENFORCEMENT: Record<string, {
  enforcement: ConstraintEnforcement;
  note: string;
}> = {
  'domainWeights.minItems': {
    enforcement: 'hard',
    note: 'Assembly FAILS if any domain minimum cannot be met.',
  },
  'domainWeights.maxItems': {
    enforcement: 'hard',
    note: 'No domain may exceed its maximum.',
  },
  'domainWeights.targetShare': {
    enforcement: 'advisory',
    note: 'Used to steer fill order between the minimum and maximum; not a failure condition.',
  },
  minimumLength: {
    enforcement: 'hard',
    note: 'A form shorter than this fails assembly.',
  },
  maximumLength: {
    enforcement: 'hard',
    note: 'Enforced via domain maxima and target length.',
  },
  targetLength: {
    enforcement: 'advisory',
    note: 'Aimed for; a shorter form above the minimum is accepted.',
  },
  intendedGrades: {
    enforcement: 'hard',
    note: 'v0.58: checked before assembly in prepareGrowthAdministration.',
  },
  allowedFormats: {
    enforcement: 'hard',
    note: 'Items in other formats are filtered out.',
  },
  maxExposurePerWindow: {
    enforcement: 'hard',
    note: 'Over-exposed items are excluded.',
  },
  'candidate.gradeRange': {
    enforcement: 'hard',
    note: 'Items outside the target grade band are excluded.',
  },
  'candidate.enemyItemIds': {
    enforcement: 'hard',
    note: 'An item blocks its declared enemies from the same form.',
  },
  language: {
    enforcement: 'hard',
    note: 'Only same-language candidates are considered.',
  },
  cognitiveDemandTargets: {
    // Stated plainly rather than quietly implied.
    enforcement: 'advisory',
    note: 'NOT enforced in v0.58. The pilot bank is too small for a demand distribution to be meaningful, and enforcing it would fail assembly for a reason that carries no evidence. Revisit once the bank exists and the framework confirms the requirement.',
  },
  difficultyTarget: {
    enforcement: 'advisory',
    note: 'Author-assigned difficulty is not calibrated, so the mean is reported as a note and never fails assembly. Enforcing it would manufacture false psychometric precision.',
  },
  'domainWeights (unresolved constructs)': {
    enforcement: 'documentation_only',
    note: 'Domains for constructs the framework has not resolved cannot appear in an approved blueprint at all — see pilotFrameworkAuthorization.',
  },
};

/** Fields the assembler will fail on. */
export function hardConstraints(): string[] {
  return Object.entries(BLUEPRINT_CONSTRAINT_ENFORCEMENT)
    .filter(([, v]) => v.enforcement === 'hard')
    .map(([k]) => k);
}

/** Fields that are reported but never fail assembly. */
export function advisoryConstraints(): string[] {
  return Object.entries(BLUEPRINT_CONSTRAINT_ENFORCEMENT)
    .filter(([, v]) => v.enforcement === 'advisory')
    .map(([k]) => k);
}
