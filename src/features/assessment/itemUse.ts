...
export type ItemExposureRecord = {
  itemId: string;
  /** Total formal administrations. Retained for backward compatibility
   *  with records written before v0.59; equals fieldTest + operational. */
  growthAdministrations: number;
  /** v0.59 §9 — split out, because the two matter differently.
   *  Field-test exposure informs calibration sample size; operational
   *  exposure drives security, rotation, and retirement.
   *
   *  OPTIONAL: records written before v0.59 do not have them, and
   *  readers must treat absence as zero rather than as an error. */
  fieldTestAdministrations?: number;
  operationalAdministrations?: number;
...
/** v0.59 §9 — contexts that count as formal administration. */
...
export function recordExposure(
  ...
  const prev: ItemExposureRecord = log[itemId] ?? {
    ...
    fieldTestAdministrations: 0,
    operationalAdministrations: 0,
    instructionalAdministrations: 0,
    lastAdministeredAt: null,
  };
  // Legacy records lack the split fields; default them rather than
  // producing NaN.
  const fieldTest = prev.fieldTestAdministrations ?? 0;
  const operational = prev.operationalAdministrations ?? 0;

  const isFieldTest = context === 'growth_field_test';
  const isOperational = context === 'growth_operational';
  // 'growth' is the legacy generic value; treated as field test, since
  // no operational administration has ever run.
  const isFormal = isFieldTest || isOperational || context === 'growth';

  return {
    ...log,
    [itemId]: {
      ...prev,
      growthAdministrations: prev.growthAdministrations + (isFormal ? 1 : 0),
      fieldTestAdministrations:
        fieldTest + (isFieldTest || context === 'growth' ? 1 : 0),
      operationalAdministrations: operational + (isOperational ? 1 : 0),
      instructionalAdministrations:
        prev.instructionalAdministrations + (isFormal ? 0 : 1),
