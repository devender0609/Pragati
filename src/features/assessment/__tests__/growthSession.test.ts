...
import {
  legacyBuildGrowthPoolForCompatibilityOnly as buildGrowthPool, assertNoLeak, GROWTH_RULES, PRACTICE_RULES, rulesFor,
  logGrowthExposure, buildGrowthReport, REPORT_STATUS_LINE,
  FORBIDDEN_REPORT_CLAIMS, mayInterpretDomain,
  isLegacyGrowthAssignmentActive, activeLegacyAssignmentFor,
  type LegacyGrowthAssignment,
} from '../growthSession';
...
describe('§7 assignment drives whether Growth appears at all', () => {
  const a: LegacyGrowthAssignment = {
  ...
  it('is active only inside its window', () => {
    expect(isLegacyGrowthAssignmentActive(a, 500)).toBe(false);
    expect(isLegacyGrowthAssignmentActive(a, 1500)).toBe(true);
    expect(isLegacyGrowthAssignmentActive(a, 2500)).toBe(false);
  });

  it('finds nothing for a student in no classroom', () => {
    expect(activeLegacyAssignmentFor([a], null, 1500)).toBeNull();
  });

  it('finds nothing for a different classroom', () => {
    expect(activeLegacyAssignmentFor([a], 'room-b', 1500)).toBeNull();
  });

  it('finds the assignment for the right classroom in-window', () => {
    expect(activeLegacyAssignmentFor([a], 'room-a', 1500)?.id).toBe('a1');
  });
});
