...
import { officialChapterLearnState, LEARN_STATE_LABEL } from './eligibilityPolicy';
  ...
  return OFFICIAL_CHAPTERS.filter(
    ...
    .map((c) => {
      ...
      return {
        ...
        // v0.77 §10 — the wording is no longer decided here. One policy
        // function answers "what is this chapter for a student", and
        // every surface reads its label, so a fix in one place cannot
        // leave a stale claim in another.
        statusLine: LEARN_STATE_LABEL[officialChapterLearnState(c.officialChapterId)],
      };
    });
}
