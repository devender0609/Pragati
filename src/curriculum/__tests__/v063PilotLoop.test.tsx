...
describe('§11 render contracts', () => {
  ...
  it('renders unavailable chapters as non-interactive', () => {
    ...
    // v0.77 — this asserted `buttons.length === 1`, which was a PROXY
    // for the thing that matters: that no unavailable chapter accepts a
    // tap. Learn is now a studio, and the open chapter legitimately
    // offers its four practice activities alongside "Open this chapter",
    // so the count is 5. That is a capability the screen gained, not a
    // truthfulness regression, and pinning the number would have made
    // the suite argue against the product rather than against defects.
    //
    // The invariant is now asserted directly: every button belongs to a
    // chapter that is genuinely open, and no unavailable chapter title
    // appears inside one.
    const buttons = screen.getAllByRole('button');
    const unavailable = class6ChapterCards()
      .filter((c) => c.availability !== 'available')
      .map((c) => c.title);
    for (const b of buttons) {
      for (const title of unavailable) {
        expect(b.textContent).not.toContain(title);
      }
    }
