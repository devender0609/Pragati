...
describe('§14 the v0.76 report agrees with the v0.76 release', () => {
  ...
  it('states the version it was written for', () => {
    // v0.77 — this asserted package.json still read 0.76.0, which made a
    // version bump fail a test about a shipped report. The guard's job is
    // that V0.76_REPORT.md describes v0.76 accurately, not that the repo
    // stays on v0.76 forever. It now checks the report, and the current
    // release has its own guard.
