...
export type SectionEligibility = {
  ...
  /** Student-eligible LEARN content for THIS official section. Legacy
   *  skill lessons never satisfy this — see the note at its derivation. */
  hasEligibleLearn: boolean;
  /** A legacy skill lesson exists and can be opened as related practice. */
  hasLegacyLesson: boolean;
...
export function sectionEligibility(
  ...
  // ===================================================================
  // v0.77 §9 — THE AVAILABILITY DEFECT, FIXED.
  //
  // This line used to read `demoReady || legacyLesson`, which counted an
  // older Pragati skill lesson as Learn content for an official Ganita
  // Prakash section. Every student surface downstream then said "Ready
  // to learn" about Fractions, when nothing from the current textbook is
  // student-eligible: nine §7.x sections are authored drafts, none
  // reviewed, none published.
  //
  // A legacy lesson is real content and a student can genuinely open it.
  // What it is not is the chapter's lesson. So it now produces PRACTICE
  // availability, never Learn availability, and it is surfaced under
  // "related practice" wording everywhere.
  //
  // `hasLegacyLesson` is kept on the profile because the routing still
  // needs to know the activity exists — the fix removes a false claim,
  // not a capability.
  // ===================================================================
  const hasEligibleLearn = demoReady;
  ...
  else if (hasEligiblePractice || legacyLesson) availability = 'practice_available';
...
export function getStudentChapterAvailability(
  ...
  const hasEligibleLearn =
    sectionsForLearn.length > 0
      ...
      : false;
  // v0.77 §9 — a chapter has a legacy lesson if any of its sections
  // does. Kept separate from Learn availability, and never promoted to
  // it: this is what makes "related practice" routable without the
  // chapter claiming to teach the textbook.
  const hasLegacyLesson =
    sectionsForLearn.length > 0
      ? sectionsForLearn.some(
          (s) => sectionEligibility(s.officialSectionId).hasLegacyLesson
        )
  ...
  return {
    ...
    hasEligiblePractice,
    availability:
      hasEligibleLearn || hasEligiblePractice || hasLegacyLesson
        ? 'available'
        : 'not_available_yet',
  };
}
...
// ---------------------------------------------------------------------------
// v0.77 §10 — ONE POLICY, FOUR STATES.
// ---------------------------------------------------------------------------
//
// Home, Learn, the chapter landing, Practice, Progress and next-action
// all used to derive their own wording from their own reading of
// eligibility, which is how "Ready to learn" survived in one place after
// being fixed in another. There is now exactly one function that decides
// what a chapter IS for a student, and every surface asks it.
//
// The four states are deliberately about CONTENT, not about workflow. A
// student never learns that a draft exists or that a review is pending;
// they learn that lessons are being prepared.

export type OfficialLearnState =
  | 'official_learn_available'
  | 'related_practice_available'
  | 'official_lessons_preparing'
  | 'nothing_available';

export function officialChapterLearnState(
  officialChapterId: string
): OfficialLearnState {
  const e = getStudentChapterAvailability(officialChapterId);
  if (e.hasEligibleLearn) return 'official_learn_available';
  if (e.hasEligiblePractice || e.hasLegacyLesson)
    return 'related_practice_available';
  // "Being prepared" must mean something was actually written. A chapter
  // whose sections exist in the registry but have no authored content is
  // not being prepared, it simply is not there yet — and telling a
  // student otherwise is the same class of false promise this release
  // exists to remove. The distinction comes from the section's own
  // withheld reason, which already records whether a draft exists.
  const authored = sectionsForChapter(officialChapterId).some(
    (s) =>
      sectionEligibility(s.officialSectionId).withheldReason ===
      'authored draft awaiting educator review'
  );
  return authored ? 'official_lessons_preparing' : 'nothing_available';
}

/** The one place student-facing chapter wording is decided. */
export const LEARN_STATE_LABEL: Record<OfficialLearnState, string> = {
  official_learn_available: 'Ready to learn',
  related_practice_available: 'Practice available',
  official_lessons_preparing: 'Lessons being prepared',
  nothing_available: 'Not available yet',
};

/** The one place the student's primary action is decided. */
export function learnStateCta(
  state: OfficialLearnState,
  chapterTitle: string,
  hasUnfinished: boolean
): string {
  if (hasUnfinished) return 'Continue practice';
  switch (state) {
    case 'official_learn_available':
      return 'Start learning';
    case 'related_practice_available':
      return `Try ${chapterTitle} practice`;
    case 'official_lessons_preparing':
      return `Explore ${chapterTitle}`;
    default:
      return 'Explore your curriculum';
  }
}
