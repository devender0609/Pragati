...
export function submissionToCurriculum(
  ...
  return {
    ...
    authority: 'NCERT',
    // v0.78 §1 — a manual import is a person reading the primary
    // document, so both the title and the structure it carries are
    // verified by the same act. If that ever stops being true, this is
    // the line to change, not the type.
