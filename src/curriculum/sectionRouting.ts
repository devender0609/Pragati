...
export function openableSectionTarget(
  ...
  // v0.77 §9 — this used to read `hasEligibleLearn`, which was true for
  // a legacy skill lesson. Now that a legacy lesson produces PRACTICE
  // availability rather than Learn availability, the gate has to name
  // what it is actually gating: a legacy activity exists and can be
  // opened. The route is unchanged and the provenance it returns is
  // unchanged — only the false Learn claim was removed.
  if (!e.hasLegacyLesson) return null;
