  ...
  return (
    <>
      ...
      <nav
        ...
        className="fixed inset-x-0 bottom-0 z-30 border-t border-paper-300 bg-paper-50/95 backdrop-blur sm:hidden"
      >
...
export function DesktopNavigation<Id extends string>({
  ...
  return (
    <nav
      role="tablist"
      aria-label="Primary navigation"
      className="hidden flex-wrap gap-1 rounded-full bg-ink-50 p-1 text-sm font-semibold sm:inline-flex"
    >
      {items.map((item) => {
        const isActive = item.id === active;
        return (
          <button
            ...
            aria-selected={isActive}
            onClick={() => onSelect(item.id)}
            className={`inline-flex min-h-[44px] items-center gap-1.5 rounded-full px-4 py-1.5 ${MOTION.fast} ${FOCUS_RING} ${
              isActive
                ? 'bg-white text-ink-900 shadow-sm'
                : 'text-ink-500 hover:text-ink-900'
            }`}
          >
            <span className={`hidden sm:inline ${isActive ? 'text-brand-600' : 'text-ink-300'}`}>
              {item.icon}
            </span>
            {item.label}
            {typeof item.badge === 'number' && item.badge > 0 && (
