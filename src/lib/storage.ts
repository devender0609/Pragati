...
const ASSIGNMENTS_KEY = 'pragati.assignments.v1';
const LAST_EXPORT_KEY = 'pragati.last_export.v1';
...
// Assignments (v0.11)
// ---------------------------------------------------------------------------
export function loadAssignments(): AssessmentAssignment[] {
  return safeRead<AssessmentAssignment>(ASSIGNMENTS_KEY);
}

export function getActiveAssignments(): AssessmentAssignment[] {
  return loadAssignments()
    .filter((a) => a.active)
    .sort((a, b) => b.createdAt - a.createdAt);
}

export function getMostRecentActiveAssignment(): AssessmentAssignment | null {
  return getActiveAssignments()[0] ?? null;
}

export function saveAssignment(assignment: AssessmentAssignment): void {
...
export function deleteAssignment(assignmentId: string): void {
  const remaining = loadAssignments().filter((a) => a.id !== assignmentId);
  safeWrite(ASSIGNMENTS_KEY, remaining);
}

// ---------------------------------------------------------------------------
// Last export timestamp (v0.11)
// ---------------------------------------------------------------------------
// Used by the workflow's "Export pilot data" step to mark itself complete.
...
//             lack the v2 fields.
//   3 → v0.10 (adds alignmentBySkill + alignmentByItem snapshots and an
//             explicit prototypeNotice string). Older bundles still parse.
//   4 → v0.11 (adds assignments + lastExportedAt + readinessSummary).
//             Older bundles still parse.
export type ExportBundle = {
  exportedAt: string;       // ISO timestamp
  app: 'pragati';
  schemaVersion: 4;
  // v0.10: a short notice spelling out what the export IS NOT.
  ...
  // The summaries below are computed snapshots, not stored state. Caller
  // passes them in so the export bundle reflects the SAME plan / flags /
  // alignment the teacher is looking at on screen.
  teachingPlanSummary?: unknown;
  itemQualityFlags?: unknown;
  alignmentBySkill?: unknown;
  alignmentByItem?: unknown;
  readinessSummary?: unknown;
};

const PROTOTYPE_NOTICE =
  'CBSE/NCERT-informed prototype — mapped to a draft skill framework. ' +
  'NOT an official CBSE alignment, NOT a calibrated assessment, NOT a teacher-validated mapping. ' +
  'A CBSE Class 6 maths teacher should review every "medium" or "needs_teacher_review" item before any pilot use.';
...
export function buildExportBundle(extras?: {
  ...
  return {
    ...
    alignmentBySkill: extras?.alignmentBySkill,
    alignmentByItem: extras?.alignmentByItem,
    readinessSummary: extras?.readinessSummary,
  };
}
