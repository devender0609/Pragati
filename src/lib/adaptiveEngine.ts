...
// ---------------------------------------------------------------------------
// v0.27 — Blueprint-aware execution.
// ---------------------------------------------------------------------------
// A blueprint carries its own minItems / maxItems and a list of target
// modules / skills. The functions below let callers run a session that
// respects those numbers instead of the hard-coded 8 / 10, without
// changing the existing runSession path used by legacy callers.
//
// NOTE: this is still a heuristic engine (not IRT). The blueprint decides
// scope + length; the routing rule is unchanged.

import type { AssessmentBlueprint } from '../curriculum';
import { getItemsForSkill, getSkills } from '../curriculum';

export type BlueprintSessionConfig = {
  minItems: number;
  maxItems: number;
  eligibleItems: Item[];
};

// Given a blueprint, gather every eligible item from its target modules
// or skills. Never mixes across grades or subjects — the blueprint's own
// gradeId + subjectId are the scope.
export function collectItemsForBlueprint(bp: AssessmentBlueprint): Item[] {
  const skillIds: string[] = bp.skillIds
    ? [...bp.skillIds]
    : (bp.moduleIds ?? []).flatMap((mid) => getSkills(mid).map((s) => s.id));
  const seen = new Set<string>();
  const out: Item[] = [];
  for (const sid of skillIds) {
    for (const it of getItemsForSkill(sid)) {
      if (!seen.has(it.id)) {
        seen.add(it.id);
        out.push(it);
      }
...
// Compute the runtime session config from a blueprint + optional prior
// history. Throws if the blueprint's targets don't yield enough items to
// meet its own minItems — the app must never let a student start an
// assessment with zero (or too few) eligible items.
export function buildBlueprintSession(
  bp: AssessmentBlueprint,
  priorAttemptedIds: string[] = []
): { config: BlueprintSessionConfig; pool: Item[] } {
  const eligibleItems = collectItemsForBlueprint(bp);
  if (eligibleItems.length < bp.minItems) {
    throw new Error(
      `Blueprint ${bp.id} needs at least ${bp.minItems} eligible items but the registry has ${eligibleItems.length}. Refusing to start.`
    );
  }
  // The stratified pool builder is still SESSION_SIZE-shaped internally.
  // For blueprints whose maxItems differs from SESSION_SIZE, we ask the
  // pool builder for what it can produce and then trim to maxItems.
  const rawPool = buildSessionPool(eligibleItems, priorAttemptedIds);
  const pool = rawPool.slice(0, Math.max(bp.minItems, bp.maxItems));
  return {
    config: {
      minItems: bp.minItems,
      maxItems: bp.maxItems,
      eligibleItems,
    },
    pool,
  };
...
// Blueprint-aware stopping. `shouldStop` still applies for the SESSION_SIZE
// path; blueprint sessions call this instead.
export function shouldStopBlueprint(
  state: EngineState,
  config: BlueprintSessionConfig,
  poolSize: number
): boolean {
  if (state.attemptedIds.length >= config.maxItems) return true;
  if (state.attemptedIds.length >= poolSize) return true;
  return false;
}

export function canFinishEarlyBlueprint(
  state: EngineState,
  config: BlueprintSessionConfig
): boolean {
  return state.attemptedIds.length >= config.minItems;
}
