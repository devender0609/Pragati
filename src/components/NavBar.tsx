...
export function NavBar({
  ...
  // v0.69 §3 — the mode accent now surfaces ONLY in the active
  // dashboard pill. The logo is constant: a product's identity should
  // not change colour depending on who is signed in, and the mode is
  // already stated in words two elements away.
  const accentPill =
    appMode === 'teacher'
      ? 'bg-brand-50 text-brand-700 ring-brand-200'
      : 'bg-learn-50 text-learn-700 ring-learn-200';

  return (
    <header className="sticky top-0 z-10 border-b border-paper-300/80 bg-paper-50/90 backdrop-blur supports-[backdrop-filter]:bg-paper-50/75">
      <div className="mx-auto flex max-w-[84rem] items-center justify-between gap-3 px-4 py-3 sm:py-4">
        <button
          onClick={onNavLanding}
          className="flex min-h-[44px] items-center gap-2 text-left"
        >
          {/* v0.69 §3 — the real mark replaces the letter P in a blue
              square, which is the placeholder every internal tool ships
              with. `accentLogo` is retained below for the mode pill; the
              mark itself is constant, because a product's identity
              should not change colour depending on who is signed in. */}
          <PragatiMark className="h-9 w-9 shadow-sm" />
          <div className="hidden sm:block">
            <div className="font-display text-[1.05rem] font-bold tracking-tight text-ink-900">
              Pragati
            </div>
          ...
          <div className="block font-display text-[1.05rem] font-bold tracking-tight text-ink-900 sm:hidden">
            Pragati
          </div>
        </button>
