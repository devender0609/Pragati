...
export type CommonMistake = {
  /**
   * v0.77.2 §1/§2 — the same misconception, said to the student.
   *
   * `why` and `fix` are written to an adult: they explain what a learner
   * is thinking and tell a teacher what to do about it. Shown to a Class
   * 6 student they read as a report about that student, which is the
   * audience error §1 names.
   *
   * This field is the STUDENT reading of the same record. It adds no
   * mathematical claim — every line restates the authored `example` and
   * `fix` in second person — and it is optional, so a lesson without it
   * simply shows the student less rather than showing them the teacher's
   * copy. New student-facing wording, and it should go past a Class 6
   * teacher with the rest of the FR.02 draft.
   */
  student?: {
    /** What can go wrong, in the student's own case. */
    watch: string;
    /** What to do instead. Short, concrete, non-judgmental. */
    tryThis: string;
  };
  // Short pattern label.
  pattern: string;
    ...
    commonMistakes: [
      {
        ...
        student: {
          watch: 'You might count just the shaded pieces and write "3".',
          tryThis:
            'Count all the equal parts first, then the shaded ones. Say it as "3 out of 5".',
        },
      ...
      },
      {
        ...
        student: {
          watch:
            'You might name a fraction when the pieces are not all the same size.',
          tryThis:
            'Check the pieces are equal before you name a fraction. If they are not, split the whole again.',
        },
        example:
      ...
      },
      {
        ...
        student: {
          watch: 'You might write the bigger number on top, like 5/3.',
          tryThis:
            'Say it as "3 out of 5". The "out of" number always goes on the bottom.',
        },
